...
var randTable = []
for (var i=0; i < 1e6; i++) {
    randTable.push(Math.floor(Math.random()*i));
}
  ...
  addPolygonTreeNodes: function (polygontreenodes) {
    ...
    do {
      ...
      if (!node.plane) {
        ...
        //let bestplane = polygontreenodes[polygontreenodes.length >> 1].getPolygon().plane
        //let bestplane = polygontreenodes[Math.floor(Math.random()*polygontreenodes.length)].getPolygon().plane
        let bestplane = polygontreenodes[randTable[polygontreenodes.length]].getPolygon().plane
